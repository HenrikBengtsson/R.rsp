...
#
# @title "Gets a unique temporary variable name"
#
# \description{
...
# @keyword internal
#*/###########################################################################
tempvar <- function(prefix, value, envir=parent.frame(), inherits=FALSE) {
  maxTries <- 1e6;
  ...
  throw(sprintf("Failed to generate a unique temporary variable with prefix '%s'", prefix));
} # tempvar()
